...
_run_docker_compose() {
  ...
  PHPUNIT_CONFIG_ALT="$PLUGIN_DIR/phpunit.xml.dist"
...
download_gravityview() {
  ...
  GV_LATEST_HASH=$(wget --header="Authorization: token $GH_AUTH_TOKEN" -O - -q https://api.github.com/repos/gravityview/gravityview/tags | jq '.[0].commit.sha' | tr -d '"')
...
download_gravity_forms() {
  ...
  GF_LATEST_HASH=$(wget --header="Authorization: token $GH_AUTH_TOKEN" -O - -q https://api.github.com/repos/gravityforms/gravityforms/tags | jq '.[0].commit.sha' | tr -d '"')
